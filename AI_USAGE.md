...
- Vercel 배포 시 MSW 및 라우팅 이슈 해결 요청

## 적용한 작업 범위

과제 요구사항(`requirement.md`, `openapi.yaml`)을 기반으로 아래 영역에서 Claude와 대화하며 코드를 작성/검토했습니다.
...
- [x] 폰트(Pretendard), 색상 토큰, 아이콘, 레이아웃 등 심미적 요소는 AI 제안을 참고하되 직접 최종 결정
- [x] `openapi.yaml` 스펙과 실제 MSW 핸들러 응답 구조가 정확히 일치하는지 직접 대조 확인
- [x] `npm install` 및 `npx msw init public/ --save` 실행 후 브라우저에서 MSW 정상 동작(`[MSW] Mocking enabled.` 로그, Service Worker activated 상태) 확인
- [x] 로그인/로그아웃/새로고침 세션 복구 시나리오 및 GNB 아이콘 전환 실제 동작 확인
- [x] 무한 스크롤 및 가상 스크롤 실제 동작 확인
- [x] Vercel 배포 후 새로고침 라우팅 정상 동작 확인

> ※ 위 체크리스트는 실제 확인 후 갱신 바랍니다.

## 비고

- API 서버는 MSW로 mocking (`requirement.md`의 "전문" 섹션에서 허용된 방식)
